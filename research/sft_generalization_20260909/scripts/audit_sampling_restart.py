...
from common import *
root=S/'results/evaluation/dev';pairs=[('base_restart_after_sampling','base'),('previous_self_sdpa_after_sampling_repeat','previous_self_sdpa_dispatch_v2_repeat')];out=[]
...
write(S/'audits/sampling_restart_protocol_replay.json',dict(time=time.time(),passed=True,comparisons=out,scope='Specific baseline/nonzero inference paths under current shared-device conditions; does not certify every future run or every backend.'))
