...
ap = S/'audits/balanced_ood_execution_amendment.json'
...
parents = {g: dict(v, phase='waiting_handoff_child', quiet=0) for g, v in amend['parents'].items()}
...
def retire_parent(gpu):
    ...
    state.update(phase='available', retired_time=time.time(), retirement='SIGTERM then SIGCONT, after handoff child exit0 and two quiet GPU checks')
...
def verify_gpu2_replay():
    ...
    assert not actual.exists() and not legacy.exists()
    ...
    replay_manifest = Path(parents['2']['manifest'])
    ...
    common_fields=dict(time=time.time(),passed=True,n=1000,matches=matches,
        plan_sha256=amend['supplement_plan_sha256'],actual_gpu='2',
        original_gpu2_execution_amendment_sha256=amend['gpu2_execution_amendment_sha256'],
        execution_amendment_sha256=ah,execution_script_sha256=sha(__file__),
        scope='Unchanged original GPU2 replay inference; exact verification performed by balanced execution owner after replay child exit0.')
    write(actual,common_fields)
    write(legacy,dict(common_fields,legacy_filename_only=True,original_planned_gpu='1',
        actual_replay_path=str(actual),actual_replay_sha256=sha(actual)))
