...
def main():
 p=argparse.ArgumentParser();p.add_argument('--manifest',type=Path,required=True);p.add_argument('--limit',type=int,default=500);p.add_argument('--batch',type=int,default=8);a=p.parse_args();gpu_guard()
 assert a.limit==500 and a.batch==8
 plan=json.loads((S/'results/confirmation_plan.json').read_text())
 protocol=json.loads((S/'audits/final_native_protocol_plan.json').read_text());assert sha(__file__)==protocol['native_script_sha256']
 rows=read(S/'data/dev.jsonl')[:a.limit];items=json.loads(a.manifest.read_text());torch.set_num_threads(4)
 assert sha(S/'data/dev.jsonl')==protocol['data_sha256']
 expected_names=set(plan['families'][plan['primary_family']].values())|{'base'}
 assert set(items)==expected_names and items['base'] is None
 for name,adapter in items.items():
  if adapter:assert artifact_identity(Path(adapter))['files']==plan['adapter_files_sha256'][name]
 assert artifact_identity(Path(MODEL))['files']==plan['base_model_files_sha256']
 ...
 for name,adapter in items.items():
  out=S/'results/native/final_dev500'/name
