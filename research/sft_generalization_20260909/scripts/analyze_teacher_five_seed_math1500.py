...
plan=json.loads((S/'audits/teacher_average_seed_extension_plan.json').read_text());seeds=plan['fixed_seed_list'];root=S/'results/evaluation/math_reused1500'
frozen_path=S/'audits/teacher_five_seed_math1500_extension_plan.json';frozen=json.loads(frozen_path.read_text());assert sha(S/'data/math_reused1500.jsonl')==frozen['data_sha256']
assert sha(S/'results/teacher_five_seed_math1500_extension_manifest.json')==frozen['manifest_sha256']
...
base_manifest=json.loads((root/'base/math_manifest.json').read_text());assert all(base_manifest[k]==v for k,v in frozen['protocol'].items())
...
for family,template in families.items():
 ...
 for seed in seeds:
  ...
  em=json.loads((root/name/'math_manifest.json').read_text());assert em['adapter']==frozen['all_model_identities'][name];assert all(em[k]==v for k,v in frozen['protocol'].items())
...
write(S/'audits/teacher_average_five_seed_math_reused1500.json',dict(time=time.time(),validation_extension_plan_sha256=sha(frozen_path),script_sha256=sha(__file__),n=len(base),base_correct=int(b.sum()),seeds=seeds,families=out,average_minus_epoch4=summarize(vectors['average1234']-vectors['epoch4']),plan_sha256=sha(S/'audits/teacher_average_seed_extension_plan.json'),protocol_replay_sha256=sha(S/'audits/sampling_restart_protocol_replay.json'),scope='Selected-candidate five-seed followup on historically reused1500 validation, not independent confirmation; original3seeds were already observed before this4model extension. Everyseed included, no endpoint/seed switching. FinalOOD/full5000 still governed by separatefreeze.'))
