...
pairs = [
    ('teacher32_all_qkvo16_dft_lr1e5_s43_epoch4', 'teacher32_all_qkvo16_lr1e5_s43_epoch4',
     'Same32B targets, seed, LR1e-5 and fixedepoch4: DFT versus CE'),
    ('teacher32_all_qkvo16_dft_lr5e5_s43_epoch4', 'teacher32_all_qkvo16_lr5e5_s43_epoch4',
     'Same32B targets, seed, LR5e-5 and fixedepoch4: DFT versus CE'),
    ('sample_all_dft_lr1e5_s43_epoch2', 'sample_all_lr1e5_s43_epoch2',
     'Same sampled-student targets, seed, LR1e-5 and fixedepoch2: DFT versus CE'),
    ('sample_all_dft_lr5e5_s43_epoch2', 'sample_all_lr5e5_s43_epoch2',
     'Same sampled-student targets, seed, LR5e-5 and fixedepoch2: DFT versus CE')]
names = list(dict.fromkeys(['base'] + [n for t,c,_ in pairs for n in (t,c)]))
root = S/'results/evaluation/dev'
out = S/'results/dft_source_control_analysis.json'
...
while not all((root/n/'math_summary.json').exists() for n in names):
    ...
    time.sleep(20)
auditpath = S/'audits/scoring_dft_source_controls.json'
if not auditpath.exists():
    command = [sys.executable, str(S/'scripts/score_sensitivity.py'), '--dataset',
               'dev', '--tag', 'dft_source_controls', '--models', *names]
    with (S/'logs/strict_dft_source_controls.log').open('w') as log:
...
write(out,dict(time=time.time(),script_sha256=sha(__file__),n=500,seed=43,
               ...
               prospective_plan_sha256=sha(S/'audits/dft_interaction_plan.json'),
               ...
               scope='Fixed prospective pilot endpoints; single seed43 and unadjusted intervals. '
                     'The two LRs are not independent training seeds. Comparisons match targets within each '
                     'source, not across sources. No held-out general efficacy claim.'))
