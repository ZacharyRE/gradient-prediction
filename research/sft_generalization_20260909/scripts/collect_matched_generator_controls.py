...
pairs = [
    ('teacher32_common_teacher7_lr1e5_s43_epoch4', 'teacher7_common_teacher32_lr1e5_s43_epoch4',
     'Same1465 questions/order, seed43, LR1e-5 and fixedepoch4:32B versus7B generator targets'),
    ('teacher_common_lr1e5_s43_epoch4', 'sample_common_lr1e5_s43_epoch4',
     'Same1429 questions/order, seed43, LR1e-5 and fixedepoch4:7B versus sampled-student targets'),
    ('teacher_common_lr5e5_s43_epoch4', 'sample_common_lr5e5_s43_epoch4',
     'Same1429 questions/order, seed43, LR5e-5 and fixedepoch4:7B versus sampled-student targets')]
names = list(dict.fromkeys(['base'] + [n for t,c,_ in pairs for n in (t,c)]))
root = S/'results/evaluation/dev'
out = S/'results/matched_generator_control_analysis.json'
...
while not all((root/n/'math_summary.json').exists() for n in names):
    ...
    time.sleep(20)
auditpath = S/'audits/scoring_matched_generator_controls.json'
if not auditpath.exists():
    command = [sys.executable, str(S/'scripts/score_sensitivity.py'), '--dataset',
               'dev', '--tag', 'matched_generator_controls', '--models', *names]
    with (S/'logs/strict_matched_generator_controls.log').open('w') as log:
...
write(out,dict(time=time.time(),script_sha256=sha(__file__),n=500,seed=43,
               ...
               prospective_dispatch_sha256=sha(S/'audits/matched_source_dispatch_priority.json'),
               ...
               scope='Fixed prospective pilot endpoints; single seed43 and unadjusted intervals. '
                     'Two LRs are not independent seeds. The target source bundles content, process and length; '
                     'this is not a pure parameter-count or pure writing-style intervention. No held-out general efficacy claim.'))
