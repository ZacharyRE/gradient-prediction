...
from anchored_objective import anchored_backward, reference_signature
from common import *
...
def main():
 ...
 p.add_argument('--objective',choices=['token_mean','dft'],default='token_mean');p.add_argument('--modules',choices=['qkvo','all'],default='qkvo');p.add_argument('--rank',type=int,default=16);p.add_argument('--alpha',type=int,default=16)
 p.add_argument('--kl-coefficient',type=float,default=0.);p.add_argument('--initial-attention',type=Path);p.add_argument('--probe-files',nargs='*',default=[str(S/f'data/probe_{x}.jsonl') for x in ['raw','sample','teacher']]);p.add_argument('--max-length',type=int,default=2048);p.add_argument('--resume-from',type=Path);a=p.parse_args();gpu_guard();assert a.batch%4==0;assert a.kl_coefficient>=0 and a.resume_from is None
 ...
 manifest['anchored_helper_sha256']=sha(S/'scripts/anchored_objective.py');manifest['forked_original_train_sha256']='845579c65e6e037942a52f560eb79d0b910c4ca385cc78b69874dedac501c31c';shutil.copy2(S/'scripts/anchored_objective.py',out/'anchored_objective_source.py')
 ...
 anchored_audit={};reference_before=reference_signature(model,collator([ds[0]])) if a.kl_coefficient>0 else None
 for epoch in range(start_epoch,min(a.stop,a.epochs)+1):
  ...
  while True:
   ...
   m=anchored_backward(model,window,a.objective,a.kl_coefficient,anchored_audit) if a.kl_coefficient>0 else (accumulate_gradients(model,window,'cuda','token_mean','bfloat16') if a.objective=='token_mean' else custom_backward(model,window,a.objective))
 ...
 reference_after=reference_signature(model,collator([ds[0]])) if a.kl_coefficient>0 else None
 assert reference_before==reference_after
 write(out/'anchoring_audit.json',dict(coefficient=a.kl_coefficient,first_batch=anchored_audit,reference_signature_before=reference_before,reference_signature_after=reference_after,reference_unchanged=reference_before==reference_after,reference_signature_scope='First training example, three token positions, full vocabulary; actual frozen parameters excluded from optimizer. Not a full-model-state hash.',zero_coefficient_uses_original_backward_path=a.kl_coefficient==0))
